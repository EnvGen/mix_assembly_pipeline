...
#CPUS and Memory when using HPC
Thr_small = config["performance"]["Small_jobs"]["Threads"]
unit_mem_small = config["performance"]["Small_jobs"]["memory_per_cpu"]

Thr_big = config["performance"]["Big_jobs"]["Threads"]
unit_mem_big = config["performance"]["Big_jobs"]["memory_per_cpu"]

memsmall = unit_mem_small*Thr_small
membig = unit_mem_big*Thr_big
slurm_ext = config["performance"]["slurm_extra"]

chunks=int(config["n_chuncks_annotations"])#n_chuncks(int(config["n_chuncks_annotations"]),int(Thr_big) )
chunkst=int(config["n_chuncks_taxonomy"]) #n_chuncks(int(config["n_chuncks_taxonomy"]),int(Thr_big) )
...
rule unzip_and_check_ind_contigs_files:
    ...
    threads: Thr_small
    resources:
        runtime = lambda wildcards, attempt: attempt*60 * 4, mem_mb=memsmall
...
rule ind_proteins:
    ...
    threads: Thr_small
    resources:
        runtime = lambda wildcards, attempt: attempt*60 * 8, mem_mb=memsmall
    shell:  """
...
rule co_proteins_chunck:
    ...
    threads: Thr_small
    resources:
        runtime = lambda wildcards, attempt: attempt*60 * 9, mem_mb=memsmall
    shell:  """
...
rule merge_gene_calling_co_assembly:
    ...
    threads: Thr_small
    resources:
        runtime = lambda wildcards, attempt: attempt*60 * 24, mem_mb=memsmall
    shell:  """
...
rule cluster_ind:
    ...
    threads: Thr_big
    resources:
        runtime = lambda wildcards, attempt: attempt*60 * 36, mem_mb=membig, slurm_extra=slurm_ext
    shell:  """
              mkdir -p {params.tmp}
...
rule mix_in_chunks:
    ...
    threads: Thr_small
    resources:
        runtime = lambda wildcards, attempt: attempt*60 * 6, mem_mb=memsmall
    shell:  """
                n=$(grep -c ">" {input})
...
rule rfam_chunck:
    ...
    conda: "mix_gc_env"
    resources:
        runtime = lambda wildcards, attempt: attempt*60 * 240, mem_mb=membig, slurm_extra=slurm_ext
...
rule remove_false_positive:
  ...
  threads: Thr_small
  resources:
        runtime = lambda wildcards, attempt: attempt*60 * 96, mem_mb=memsmall
  shell:  """
...
rule pfam:
    ...
    conda: "mix_gc_env"
    resources:
        runtime = lambda wildcards, attempt: attempt*60 * 120, mem_mb=membig, slurm_extra=slurm_ext
...
rule dbcan:
    ...
    conda: "mix_gc_env"
    resources:
        runtime = lambda wildcards, attempt: attempt*60 * 48, mem_mb=membig, slurm_extra=slurm_ext
...
rule eggnog:
    ...
    threads: Thr_big
    resources:
        runtime = 60 * 240, mem_mb=membig, slurm_extra=slurm_ext
...
rule mix_contigs:
    ...
    threads: Thr_big
    resources:
        runtime = lambda wildcards, attempt: attempt*60 *24, mem_mb=membig, slurm_extra=slurm_ext
...
if config["include_CAT_GTDB_taxonomy"]:
    rule download_gtdb_for_CAT:
        ...
        threads: Thr_small
        resources:  runtime = lambda wildcards, attempt: attempt*60 * 50, mem_mb=memsmall
    ...
    rule cat_contigs_chunk:
        ...
        threads: Thr_big
        resources: runtime = lambda wildcards, attempt: attempt*60*24*2, mem_mb=membig, slurm_extra=slurm_ext
    ...
    rule CAT_concat:
        ...
        threads: Thr_small
        resources: runtime = lambda wildcards, attempt: attempt*60 * 5, mem_mb=memsmall
    ...
    rule CAT_krona_genes:
        ...
        threads: 4
        resources:
                runtime = lambda wildcards, attempt: attempt*60 * 4, mem_mb=unit_mem_small*4
...
if config["taxonomy_DB"] == "gtdb":
    ...
    rule download_gtdb_ref_proteins:
      	...
      	threads: Thr_small
      	resources: runtime = lambda wildcards, attempt: attempt*60 * 10, mem_mb=memsmall
    ...
    rule taxonomy_gtdb:
      ...
      threads: Thr_big
      resources:
        	runtime = lambda wildcards, attempt: attempt*60 * 96, mem_mb=membig, slurm_extra=slurm_ext
      params: p=config["mmseqs_taxonomy_params"], t=config["tmp_dir"]+"/taxo_gtdb_{chunkt}", o="Taxonomy_gtdb/{chunkt}_Mix_easy_tax"
      shell:  """
                mkdir -p {params.t}
...
if config["taxonomy_DB"] == "uniprot" and config["uniprot"]["by_chuncks"] == False :
   ...
   rule taxonomy_uniprot:
        ...
        threads: Thr_big
    	resources:
        	runtime = lambda wildcards, attempt: attempt*60 * 100, mem_mb=membig, slurm_extra=slurm_ext
   ...
   rule taxonomy_uniport_tsv:
        ...
        threads: 4
    	resources:
        	runtime = lambda wildcards, attempt: attempt*60 * 8, mem_mb=unit_mem_small*4
...
if config["taxonomy_DB"] == "gtdb" or config["taxonomy_DB"] == "uniprot" and config["uniprot"]["by_chuncks"] == True :
  ...
  rule filtre_Virus:
    ...
    threads: Thr_small
    resources: runtime = 60 * 15, mem_mb=memsmall
  ...
  rule taxonomy_virus:
    ...
    threads: Thr_big
    resources:
        	runtime = lambda wildcards, attempt: attempt*60 * 10, mem_mb=membig, slurm_extra=slurm_ext
  ...
  rule taxonomy_Euka:
    ...
    resources:
        	runtime = lambda wildcards, attempt: attempt*60 * 10,
                mem_mb=membig, slurm_extra=slurm_ext
...
if config["taxonomy_DB"] == "gtdb":
  rule taxonomy_combined:
    ...
    threads: 4
    resources:
        runtime = lambda wildcards, attempt: attempt*60 * 6, mem_mb=unit_mem_small*4
...
rule table_rep_clusters:
    ...
    threads: 10
    resources:
        runtime = lambda wildcards, attempt: attempt*60 * 4, mem_mb=unit_mem_small*10
